...
# Hardcoded values:
db_path = r"C:\Users\AlexPHD\Dartmouth College Dropbox\Alexander Carney\darpa_alex_local\EP_gui_application\databases\NR_R_FRI_OVN_HAILEY.db"
experiment_id = "e4b2b4ed-0182-4abf-b3a5-d63f5e4b7ef3"
readout_type = "normal"
...
for N in range(N_min, N_max + 1):
    ...
    # Select the Nth best match (largest detuning first)
    best_match = sorted_settings_df.iloc[N - 1]
    db_cavity_freq = best_match["omega_C"]  # in GHz
    db_w_y = best_match["omega_Y"]  # in GHz
    db_gamma_vec = np.array([best_match["kappa_C"], best_match["kappa_Y"]])
    print("Best match from DB settings (based on detuning):")
    print(best_match)
