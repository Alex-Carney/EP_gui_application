...
def plot_individual_trace(current_value, frequencies, power_dbm, readout_type, base_folder, fit_data,
    ...
    if fit_data is not None and fit_data.get("fit_result") is not None:
        ...
        if fit_data.get("fit_type") == "single":
            # Extract center and FWHM from the single Lorentzian fit.
            ...
            ax.annotate(f"Peak: {center:.3f} GHz", (center, peak_val_db),
                        textcoords="offset points", xytext=(0, 20), ha="center", color="red")
            # Plot horizontal line representing the FWHM (linewidth)
            ...
            if linewidth is not None and not np.isnan(linewidth):
                ...
                ax.hlines(y=peak_val_db, xmin=x_left, xmax=x_right,
                          color="purple", linestyle="--", label="FWHM")
        elif fit_data.get("fit_type") == "double":
            # Plot first Lorentzian peak and its FWHM.
            try:
                ...
                if linewidth1 is not None and not np.isnan(linewidth1):
                    ...
                    ax.hlines(y=peak_val_db1, xmin=x_left1, xmax=x_right1,
                              color="purple", linestyle="--", label="FWHM 1")
            except Exception as e:
                print("Error in double fit (peak1):", e)
            # Plot second Lorentzian peak and its FWHM.
            try:
                ...
                if linewidth2 is not None and not np.isnan(linewidth2):
                    ...
                    ax.hlines(y=peak_val_db2, xmin=x_left2, xmax=x_right2,
                              color="brown", linestyle="--", label="FWHM 2")
            except Exception as e:
...
def plot_fig3(nr_power_grid, currents, frequencies, delta_df, experiment_id, settings,
    ...
    Create Figure 3 consisting of three vertically stacked subplots:
      ...
      1) Top: A colorplot with detuning on the X-axis and frequency (in GHz) on the Y-axis.
         ...
         frequency (from your NR fits) and at frequencies ±2 MHz (i.e. ±0.002 GHz) from it.
      ...
      3) Bottom: A 2D trace plot of the derivative (numerical d(Power)/d(Detuning)) versus detuning.
         The same three traces (center, center + 2 MHz, center – 2 MHz) are shown.
    ...
    target_plus = target_center + frequency_radius # +2 MHz (0.002 GHz)
    target_minus = target_center - frequency_radius  # -2 MHz (0.002 GHz)
    ...
    # (3) Bottom subplot: 2D trace plot of the derivative (d(Power)/d(Detuning)).
    ax2 = axes[2]
    # Compute numerical derivatives with respect to Delta_sorted.
    deriv_center = np.gradient(trace_center, Delta_sorted)
    deriv_plus = np.gradient(trace_plus, Delta_sorted)
    deriv_minus = np.gradient(trace_minus, Delta_sorted)
    ax2.plot(Delta_sorted, deriv_center, color=color_center, label='Center Frequency')
    ax2.plot(Delta_sorted, deriv_plus, color=color_plus, label='Center + 2 MHz')
    ax2.plot(Delta_sorted, deriv_minus, color=color_minus, label='Center - 2 MHz')
    ax2.set_xlabel("Detuning Δ (GHz)", fontsize=14)
    ax2.set_ylabel("d(Power)/d(Detuning)", fontsize=14)
    ax2.set_title("2D Trace Plot: Derivative of Power vs. Detuning", fontsize=16)
