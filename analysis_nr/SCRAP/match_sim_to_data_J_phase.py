...
from scipy.optimize import minimize  # For 2D optimization
...
trace_index = 0  # choose one trace (e.g., at the lowest current)
measured_trace_dbm = measured_power[trace_index, :]

# For extreme debugging: save a plot of the measured trace.
plt.figure()
plt.plot(measured_freqs, measured_trace_dbm)
plt.xlabel("Frequency (GHz)")
plt.ylabel("Power (dBm)")
plt.title("Measured Trace for Overlay")
plt.grid(True)
plt.savefig("EXTREME_measured_trace_for_overlay.png", dpi=400)
plt.close()
...
# Convert the measured trace from dBm to linear scale for fitting.
...
# For plotting the experimental fit.
...
# ------------------ SIMULATION TRACE & 2D COST FUNCTION ------------------
lo_freqs = np.linspace(mean_freq - FREQUENCY_RADIUS, mean_freq + FREQUENCY_RADIUS, 1000)  # GHz for simulation

# --- Modified simulation function that now accepts a phase parameter ---
def simulate_trace(J_val, phase_val):
    """
    Given a trial J_val and phase (in degrees), simulate the trace.
    """
    sim_params = ModelParams(
        ...
        phi_val=np.deg2rad(phase_val),  # Use the provided phase
    )
...
# --- New 2D cost function that depends on both J and phase ---
def cost_function_2d(params):
    """
    For a given set of parameters [J, phase], simulate the trace,
    extract its peak frequencies, and return the sum of squared differences
    between the simulated peaks and the experimentally fitted peaks.
    """
    J_val = params[0]
    phase_val = params[1]
    sim_trace = simulate_trace(J_val, phase_val)
...
# --- Optimize both J and phase ---
# Set the initial guess: J ~0.008414 and phase = 180 degrees.
initial_guess = [0.008414, 180.0]
# Define bounds: for example, J between 0.007 and 0.009, phase between 170 and 190.
bounds = [(0.007, 0.009), (170, 190)]
res2 = minimize(cost_function_2d, x0=initial_guess, bounds=bounds, method='L-BFGS-B')
best_J = res2.x[0]
best_phase = res2.x[1]
print(f"Optimal J found: {best_J:.6f}, Optimal phase found: {best_phase:.2f} with cost {res2.fun:.3e}")

# Recompute simulation trace with the optimal J and phase.
simulated_trace_opt = simulate_trace(best_J, best_phase)
simulated_trace_initial = simulate_trace(initial_guess[0], initial_guess[1])
...
plt.plot(lo_freqs, simulated_trace_opt,
         label=f"Simulated Trace (J = {best_J:.6f}, Phase = {best_phase:.1f}°)",
         color="red", linestyle="--")
plt.plot(lo_freqs, simulated_trace_initial,
         label=f"Simulated Trace (J = {initial_guess[0]:.6f}, Phase = {initial_guess[1]:.1f}°)",
...
# --- Mark the peaks for the initial J/phase values ---
...
plt.plot(initial_peaks_freq, initial_peaks_y, 'y*', markersize=12, label="Simulated Peaks (Initial J/Phase)")
